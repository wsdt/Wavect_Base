...
export const ChallengeTypeIcon: React.FunctionComponent<IChallengeTypeIconProps> = props => {
    return (
        <TouchableOpacity style={styles.touchableContainer}>
            <Tooltip popover={<Text style={globalStyles.tooltipText}>{CHALLENGE_CATEGORIES[props.type].descr}</Text>}
                     backgroundColor={GREY} height={75}>

                <Image source={CHALLENGE_CATEGORIES[props.type].icon} containerStyle={styles.imageContainer}
                       style={styles.image}/>
            </Tooltip>
        </TouchableOpacity>
    )
