...
var GetStreamParent = /** @class */ (function(_super) {
    ...
    function GetStreamParent(props) {
        ...
        _this.requestGetstreamToken = function(userId) {
            var TARGET_URI =
                App_constants_1.BACKEND_URL + "/api/v1/auth/" + userId
            fetch(TARGET_URI)
                .then(function(res) {
                    return res.json()
                })
                .then(function(data) {
                    return _this.setState({ userToken: data.token })
                })
                .catch(function(e) {
                    react_native_1.ToastAndroid.show(
                        "Could not connect to server.",
                        react_native_1.ToastAndroid.SHORT
                    )
                    console.warn(
                        "URI: " + TARGET_URI + ", warning -> " + e.toString()
                    )
                })
        }
        ...
        if (this.state.userToken !== "") {
            return (
                <expo_activity_feed_1.StreamApp
                    apiKey={GetStreamParent_secrets_1.GS_KEY}
                    appId={GetStreamParent_secrets_1.GS_APP_ID}
